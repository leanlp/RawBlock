...
function buildNodeRecap(
  node: GraphNode,
  neighborsCount: number,
  consensusRulesCount: number,
  policyRulesCount: number,
): string[] {
  const recap = [
    node.summary,
    `This concept connects to ${neighborsCount} neighboring node${neighborsCount === 1 ? "" : "s"} in the protocol graph.`,
  ];

  if (consensusRulesCount + policyRulesCount > 0) {
    recap.push(
      `Validation linkage: ${consensusRulesCount} consensus rule${consensusRulesCount === 1 ? "" : "s"} and ${policyRulesCount} policy rule${policyRulesCount === 1 ? "" : "s"}.`,
    );
  }

  if (node.securityNotes && node.securityNotes.length > 0) {
    recap.push(`Security focus: ${node.securityNotes[0]}`);
  }

  if (node.explorerLinks && node.explorerLinks.length > 0) {
    recap.push(`Hands-on follow-up available via ${node.explorerLinks.length} explorer link${node.explorerLinks.length === 1 ? "" : "s"}.`);
  }

  return recap.slice(0, 4);
}
...
export default async function AcademyNodePage({ params }: AcademyNodePageProps) {
  ...
  const recapPoints = buildNodeRecap(
    node,
    neighbors.length,
    consensusRules.length,
    policyRules.length,
  );
      ...
      <div className="mx-auto max-w-5xl space-y-8">
        ...
        <div className="grid gap-8 lg:grid-cols-12">
          <div className="space-y-8 lg:col-span-8">
            ...
            <section className="rounded-2xl border border-cyan-800/60 bg-cyan-950/20 p-5">
