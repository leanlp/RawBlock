...
export default function MinersPage() {
  ...
  const fetchData = useCallback((silent = false) => {
    ...
    fetchPrimary()
      .catch((primaryErr) => {
        console.warn(
          "Live miner feed unavailable, using snapshot fallback:",
          primaryErr,
        );
        setModeNotice(
          "Demo mode: miner data loaded from public mempool snapshot.",
        );
    ...
    <main className="min-h-screen bg-slate-950 text-slate-200 p-4 md:p-8 font-sans">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          {!loading && !error && (
            <ProvenanceBadge
              source={modeNotice ? 'Public API Fallback' : 'Live Node'}
        ...
        {!loading && !error && data && (
          <>
            {data.blocks && data.blocks.length > 0 && (
              <MinerForensicsPanel blocks={data.blocks} />
            )}
                ...
                <div className="md:hidden p-4 space-y-3">
                  {(data.blocks || []).map((block) => (
                    <Card
                      key={block.hash}
                      onClick={() =>
                        router.push(`/explorer/block/${block.hash}`)
                      }
                      className="p-4"
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span
                          className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${block.miner === "Unknown"
