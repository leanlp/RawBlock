...
import {
    ...
    BITCOIN_INITIAL_SUBSIDY_BTC,
    BITCOIN_MAX_SUPPLY_BTC,
    getBlockSubsidy as getBitcoinBlockSubsidy,
...
export function getBlockSubsidy(height: number): number {
    return getBitcoinBlockSubsidy(height);
}
...
export function validateBlock(block: BlockData): ValidationStep[] {
    ...
    steps.push({
        ...
        explanation: `Difficulty adjusts every 2016 blocks (~2 weeks) to maintain ${BITCOIN_BLOCK_TIME_MINUTES}-minute average block times. Higher difficulty = smaller target = harder to find valid hash. Bitcoin base layer throughput is roughly ~${BITCOIN_BASE_LAYER_TPS_ESTIMATE} TPS, so fee markets and L2 scaling emerge under heavy demand.`,
    ...
    steps.push({
        ...
        explanation: `The subsidy halves every ${BITCOIN_HALVING_INTERVAL_BLOCKS.toLocaleString()} blocks (~4 years). Started at ${BITCOIN_INITIAL_SUBSIDY_BTC} BTC, now at ${expectedSubsidy} BTC. This creates Bitcoin's fixed supply of ${BITCOIN_MAX_SUPPLY_BTC.toLocaleString()} BTC.`,
        status: coinbaseOutput >= expectedSubsidy ? 'pass' : 'info',
        details: {
            height: block.header.height,
            halvings: Math.floor(block.header.height / BITCOIN_HALVING_INTERVAL_BLOCKS),
